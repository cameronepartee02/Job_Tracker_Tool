...
Comparing a self-reported skills baseline against the skills extracted from these postings, roughly **39% of all skill mentions matched something I already have** — technical skills matched better (45%) than business acumen skills (19%).

The most consistently recurring gap was **GenAI/LLM application skills** (RAG, embeddings, agentic tooling) — showing up across a third of postings and concentrated most heavily at IBM specifically, one of my top target companies.

## What's next

This finding is shaping the direction of my actual second portfolio project — using it to build something that demonstrates hands-on GenAI/LLM application work, since that's the clearest evidence-backed gap between what I have and what these roles are asking for.
